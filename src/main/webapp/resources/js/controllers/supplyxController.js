...
var SupplyController = function($scope, $http, $window, $timeout) {
				...
				function(response) {
					...
					if (response.data.noReg == null) {
						...
						$timeout(function() {
							...
							$scope.qtySupply = $scope.tagLpb.qtySupplyx;
							if($scope.qtySupply == 0){
								$scope.qtySupply = $scope.tagLpb.qty;
							}
							$scope.qtyRetur = $scope.tagLpb.qtyRetur;
	...
	$scope.saveData = function(taglpb) {
		var actionUrl = "supplyx/pushe?kdGudang="+$scope.selectedWhs.kode;
		 $scope.tagLpb.qtySupplyx = $scope.qtySupply;
		 $scope.tagLpb.qtyRetur = $scope.qtyRetur;
		var resp = $http({
			method : 'POST',
			url : actionUrl,
			data : taglpb
	...
	$scope.refreshDatalist = function() {
		var actionUrl = "supplyx/supplylist.json";
		var res = $http.get(actionUrl, {
			params : {
				'kdGudang' : $scope.selectedWhs.kode
			}
